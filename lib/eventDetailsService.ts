...
const getAllExamsForSubject = (subject: string): Array<{ date: string; type: string; description: string; isPast?: boolean; isUpcoming?: boolean }> => {
  const subjectExams = getExamsForSubject(subject);

  // Find the next upcoming exam
  const upcomingExams = subjectExams.filter(exam => {
    const examDate = exam.confirmedDate ? parseISO(exam.confirmedDate) : parseISO(exam.weekEnd);
    return !isBefore(examDate, new Date());
  }).sort((a, b) => {
    const dateA = a.confirmedDate ? parseISO(a.confirmedDate) : parseISO(a.weekStart);
    const dateB = b.confirmedDate ? parseISO(b.confirmedDate) : parseISO(b.weekStart);
  ...
  return subjectExams.map(exam => {
    ...
    // Sort by date
    const dateA = subjectExams.find(exam => formatExamDate(exam) === a.date);
    const dateB = subjectExams.find(exam => formatExamDate(exam) === b.date);
    if (!dateA || !dateB) return 0;

    const parsedDateA = dateA.confirmedDate ? parseISO(dateA.confirmedDate) : parseISO(dateA.weekStart);
    const parsedDateB = dateB.confirmedDate ? parseISO(dateB.confirmedDate) : parseISO(dateB.weekStart);
...
const dummyEventDetails: Record<string, EventDetails> = {
  ...
  'Digitalni svet': {
    ...
    time: '',
    books: [
      'Digitalni svet za 2. razred - udžbenik'
    ],
    equipment: [
      'USB flash memorija',
      'Slušalice'
  ...
  'Likovna kultura': {
    ...
    time: '',
    books: [
      'Likovna kultura za 2. razred - udžbenik'
    ],
    equipment: [
      'Bojice',
      'Flomastere',
  ...
  'Muzička kultura': {
    ...
    time: '',
    books: [
      'Muzička kultura za 2. razred - udžbenik'
    ],
    equipment: [
      'Dečije orgulje (opciono)'
    ],
    allExams: []
  },
  ...
  'Dopunska nastava': {
    ...
    time: '',
    equipment: [
      'Materijali za predmet koji se dopunjuje',
      'Olovka',
      'Sveska'
    ]
  },
...
export const getEventDetails = (title: string, time: string, classType?: string): EventDetails | null => {
